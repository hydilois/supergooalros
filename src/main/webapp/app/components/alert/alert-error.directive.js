(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('supergooalrosApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-supergooalrosApp-error');
                var entityKey = httpResponse.headers('X-supergooalrosApp-params');
                if (errorHeader) {
